...
class IndexProfileResponse(BaseModel):
    ...
    num_chunks: int = Field(
        ..., description="Number of chunks processed for the profile", ge=0
    )
...
class IndexSectionRequest(BaseModel):
    ...
    section_id: str = Field(
        ...,
        description="A unique identifier for the resume section or bullet being indexed.",
        min_length=1,  # FIX: Enforce non-empty string
    )
...
class IndexSectionResponse(BaseModel):
    ...
    chunk_ids: List[str] = Field(
        ..., description="List of new chunk IDs created for this section."
    )
...
class RetrieveRequest(BaseModel):
    ...
    query_embedding: List[float] = Field(
        ..., description="Query embedding vector", min_length=384, max_length=384
    )
    top_k: int = Field(
        default=5, description="Number of top results to return", ge=1, le=100
    )
    index_namespace: IndexNamespace = Field(
        default="profile",
        description="The index to search: 'profile' for general data or 'resume_sections' for user-edited chunks.",
    )
    filter_by_section_ids: Optional[List[str]] = Field(
        default=None,
        description="Optional list of section_ids to filter results. Primarily for the 'resume_sections' namespace.",
    )
