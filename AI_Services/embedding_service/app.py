...
from faiss_index import build_index_from_db, search, rebuild_index_for_user_namespace, delete_user_index
from db import init_db, store_chunk, get_all_chunks, get_chunk_by_id, mark_user_indexed, delete_user_chunks, delete_chunks_by_section_id
from schemas import (
    EmbedRequest, EmbedResponse, IndexProfileResponse, RetrieveRequest, RetrieveResponse, 
    ChunkItem, IndexSectionRequest, IndexSectionResponse, DeleteSectionResponse
)
from chunking import chunk_text, extract_text_fields

# Global httpx client for making async requests
...
app = FastAPI(title="CVisionary Embedding Service", version="1.1.0", lifespan=lifespan)

# --- Endpoints ---

@app.post("/index/profile/{user_id}", response_model=IndexProfileResponse, tags=["Indexing"])
async def index_user_profile(user_id: str):
    ...
    try:
        ...
        response = await http_client.get(f"http://localhost:5000/profile/{user_id}")
        ...
        for source_type, source_id, text in text_fields:
            ...
            for chunk_text_content in chunks:
                ...
                store_chunk(chunk_id, user_id, 'profile', None, source_type, source_id, chunk_text_content, embedding_bytes)
                chunks_to_add.append({'chunk_id': chunk_id, 'vector': embedding_vector})
                total_chunks += 1
        ...
        return IndexProfileResponse(status=f"Profile for user {user_id} re-indexed successfully", num_chunks=total_chunks)
        
    except httpx.RequestError as e:
        raise HTTPException(status_code=503, detail=f"Failed to fetch profile from backend: {str(e)}")
...
@app.post("/index/{user_id}/section", response_model=IndexSectionResponse, tags=["Indexing"])
async def index_resume_section(user_id: str, request: IndexSectionRequest):
    ...
    try:
        ...
        for i, chunk_text_content in enumerate(chunks):
            ...
            store_chunk(
                chunk_id=chunk_id, user_id=user_id, namespace='resume_sections',
                section_id=request.section_id, source_type='user_edited',
                source_id=str(i), text=chunk_text_content, embedding_bytes=embedding_vector.tobytes()
            )
...
@app.delete("/index/{user_id}/section/{section_id}", response_model=DeleteSectionResponse, tags=["Indexing"])
...
async def retrieve_similar_chunks(user_id: str, request: RetrieveRequest):
    ...
    try:
        ...
        chunk_ids, scores = search(user_id, request.index_namespace, query_vec, request.top_k)
        ...
        for chunk_id, score in zip(chunk_ids, scores):
            ...
            if request.filter_by_section_ids and chunk_data['section_id'] not in request.filter_by_section_ids:
                continue
